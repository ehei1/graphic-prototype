...
///// refactoring
_LightImpl::_LightImpl( LPDIRECT3DDEVICE9 pDevice, LONG x, LONG y, const D3DDISPLAYMODE& displayMode, LPDIRECT3DPIXELSHADER9 pBlurShader ) : m_pBlurPixelShader( pBlurShader )
{
	// TODO 메시로 바꾸기
	ASSERT( pDevice );
...
HRESULT _LightImpl::UpdateLight( LPDIRECT3DDEVICE9 pDevice, const Setting& setting )
{
	// 텍스처 수정
	if ( m_pLightTexture && ( m_setting.lightColor != setting.lightColor || m_setting.intensity != setting.intensity ) ) {
		SAFE_RELEASE( m_pLightTexture );
...
HRESULT _LightImpl::CreateLightTextureByRenderer( LPDIRECT3DDEVICE9 pDevice, LPDIRECT3DTEXTURE9* pOutTexture ) const
{
	ASSERT( !*pOutTexture );
...
HRESULT _LightImpl::CreateLightTextureByLockRect( LPDIRECT3DDEVICE9 pDevice, LPDIRECT3DTEXTURE9* pOutTexture, const Setting& setting ) const
{
	ASSERT( !*pOutTexture );
...
HRESULT _LightImpl::UpdateLightVertex( LPDIRECT3DDEVICE9 pDevice, WORD updatingIndex, const D3DXVECTOR3& position )
{
	if ( m_pLightVertexBuffer ) {
		for ( auto index : m_lightIndices ) {
			if ( updatingIndex == index ) {
...
HRESULT _LightImpl::UpdateLightVertex( LPDIRECT3DDEVICE9 pDevice, const Points& points )
{
	if ( m_pLightVertexBuffer ) {
		ASSERT( m_lightVertices.size() == points.size() );
...
HRESULT _LightImpl::AddLightVertex( LPDIRECT3DDEVICE9 pDevice, size_t index, const D3DXVECTOR3& position )
{
	if ( m_lightVertices.size() < index ) {
		ASSERT( FALSE );
...
HRESULT _LightImpl::RemoveLightVertex( LPDIRECT3DDEVICE9 pDevice, size_t index )
{
	auto points = GetPointsFromVertices( m_lightVertices );
	auto iterator = std::next( std::cbegin( points ), index );
	ASSERT( iterator != std::cend( points ) );
...
HRESULT _LightImpl::CopyToMemory( LPDIRECT3DVERTEXBUFFER9 dest, LPVOID src, UINT size ) const
{
	ASSERT( size );
...
HRESULT _LightImpl::UpdateLightIndexBuffer( LPDIRECT3DINDEXBUFFER9* pOut, Indices& indices, LPDIRECT3DDEVICE9 pDevice, size_t vertexSize ) const
{
	// 인덱스를 만든다
	// https://en.cppreference.com/w/cpp/algorithm/generate
	auto increaseNumber = [n = 0]() mutable { return n++; };
...
HRESULT _LightImpl::UpdateLightVertexBuffer( LPDIRECT3DVERTEXBUFFER9* pOut, Vertices& vertices, LPDIRECT3DDEVICE9 pDevice, const Points& points, float falloff )
{
	ASSERT( !points.empty() );
...
_LightImpl::Points _LightImpl::GetPointsFromVertices( const Vertices& ) const
{
	Points points;
	auto addPoint = []( const CUSTOM_VERTEX& vertex ) { return vertex.position; };
	std::transform( std::cbegin( m_lightVertices ), std::cend( m_lightVertices ), std::back_inserter( points ), addPoint );
...
// TODO: use async
HRESULT _LightImpl::UpdateBlurMask( LPDIRECT3DDEVICE9 pDevice, const Vertices& vertices )
{
	if ( !m_pMaskMesh ) {
		if ( FAILED( CreateMesh( pDevice, &m_pMaskMesh, 1, 1 ) ) ) {
			return E_FAIL;
...
template<class _InIt>
D3DXVECTOR3 _LightImpl::GetCenterPoint( _InIt _First, _InIt _Last ) const
{
	assert( _First != _Last );
	auto getMidPoint = []( const auto& p0, const auto& p1 ) {
		return ( p0 + p1 ) / 2.f;
...
HRESULT _LightImpl::CreateTexture( LPDIRECT3DDEVICE9 pDevice, LPDIRECT3DTEXTURE9* pOutTexture, UINT width, UINT height )
{
	ASSERT( !*pOutTexture );
...
HRESULT _LightImpl::CreateMesh( LPDIRECT3DDEVICE9 pDevice, LPD3DXMESH* pOutMesh, UINT width, UINT height )
{
	ASSERT( !*pOutMesh );
...
HRESULT _LightImpl::Draw( LPDIRECT3DDEVICE9 pDevice )
{
	// 마스크 메시를 그린다
	if ( SUCCEEDED( pDevice->BeginScene() ) ) {
		DWORD curBlendOp = {};
...
HRESULT _LightImpl::SetSetting( LPDIRECT3DDEVICE9 pDevice, const Setting& setting )
{
	if ( m_setting != setting ) {
		UpdateLight( pDevice, setting );
...
_Freeform::_Freeform( LPDIRECT3DPIXELSHADER9 pBlurShader, const D3DDISPLAYMODE& displayMode ) : m_pBlurPixelShader{ pBlurShader }
{
	m_displayMode = displayMode;
}

HRESULT _Freeform::Draw( LPDIRECT3DDEVICE9 pDevice, float x, float y )
{
...
_EditableFreeform::_EditableFreeform( LPDIRECT3DPIXELSHADER9 pBlurShader, const D3DDISPLAYMODE& displayMode ) : _Freeform{ pBlurShader, displayMode }
{}

// 개발 용도의 imgui 창을 만든다
HRESULT _EditableFreeform::CreateImgui( LPDIRECT3DDEVICE9 pDevice, LONG xCenter, LONG yCenter, bool isAmbientMode, bool* pIsVisible )
{
	ASSERT( pIsVisible );
...
HRESULT _EditableFreeform::AddLight( LPDIRECT3DDEVICE9 pDevice, LONG x, LONG y )
{
	auto lightImpl = new _LightImpl( pDevice, x, y, m_displayMode, m_pBlurPixelShader );
...
HRESULT _EditableFreeform::RemoveLight( size_t index )
{
